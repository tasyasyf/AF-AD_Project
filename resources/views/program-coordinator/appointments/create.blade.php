<x-layouts.app title="Assign AF/AD Role">

<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h5 class="fw-bold mb-1">Assign AF/AD Role</h5>
        <div class="text-muted small">Create a course appointment for a verified AF/AD profile.</div>
    </div>
    <a href="{{ route('pc.appointments.index') }}" class="btn btn-outline-secondary btn-sm">
    ...
    <div class="col-lg-8">
        <div class="card">
            <div class="card-body p-4">
                <form method="POST" action="{{ route('pc.appointments.store') }}">
                    @csrf
                    <div class="mb-3">
                        ...
                        <select name="profile_id" class="form-select @error('profile_id') is-invalid @enderror" required>
                            ...
                            @foreach($profiles as $profile)
                                <option value="{{ $profile->id }}" {{ old('profile_id', $selectedProfile) == $profile->id ? 'selected' : '' }}>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('[data-course-code]').forEach(function (codeSelect) {
        const form = codeSelect.closest('form');
        const nameSelect = form.querySelector('[data-course-name]');
